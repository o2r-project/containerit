...
The shinyapps-package-dependencies demonstrate a (currently system dependent) alternative.
...
So far, we were able to reproduce compete R sessions regarding loaded and attached packages and mitigate some challenges towards reproducible computational research.
...
Related to specific versions is support for more package repositories, such as Bioconductor, git, BitBucket, or even local files.
...
At the time of writing this vignette, the package is a first prototype available via GitHub.
While a publication on CRAN is a goal, it should be preceeded by feedback from the useR community and ideally be accompanied by related packages, such as [harbor](https://github.com/wch/harbor/issues/5), being available on CRAN, too.
The prototype of `containerit` was developed and testet only on Ubuntu/Linux.
Before releasing a stable version on CRAN, it should be tested on Mac OS X and Windows.
...
Making `containerit` itself easier to use for endusers is a secondary goal, for example by building a graphical user interface for metadata creation.
Country locales are also not supported yet.
We may want to support other container OS (e.g. windows container or other Linux distributions).
