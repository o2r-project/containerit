...
file.copy(from = system.file("doc/sf1.Rmd",package = "sf"),
                        to = temp_workspace, recursive = TRUE)
dockerfile_object <- dockerfile(from = file.path(temp_workspace, "sf1.Rmd"), silent = TRUE, 
                                cmd = CMD_Render("sf1.Rmd"))
```
...
The `sessionInfo` object must have the name `sessionInfo` and must be the only object in the `.Rdata` file.
...
dockerfile_resources <- dockerfile(from = file.path(temp_workspace, "simple_test_script_resources"),
              copy = "script_dir",
