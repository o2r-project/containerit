...
```{r print_sf_vignette, comment=NA}
print(dockerfile_object)
```
...
unloadNamespace("sf")
df <- dockerfile(from = file.path(temp_workspace, "sessionInfo.Rdata"))
print(df)
```

Two alternative packages provide their own version of a session info, both of which are supported as well.
The following examples first load the packages `A3` and `abc` (because they are first in the alphabet), store session information objects, and then unload the packages again before creating `Dockerfile`s from the stored objects.
...
suppressPackageStartupMessages(library("A3"))
