...
```{r pre_build_df, include=FALSE}
# run command once so the next cell just uses cache!
...
```{r maintainer}
labeled_dockerfile <- dockerfile(from = empty_session(), maintainer = "Jon_Doe@example.com")
```
...
```{r label_json}
addInstruction(labeled_dockerfile) <- Label_SessionInfo(session = empty_session())
