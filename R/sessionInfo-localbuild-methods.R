docker_build = function (host = harbor::localhost, dockerfolder, new_image, wait = FALSE, no_cache = FALSE, ...) {
  stopifnot(file.exists(dockerfolder))
  docker_opts = paste("-t", new_image)
  if(no_cache)
    docker_opts = append(docker_opts, "--no-cache")
  harbor::docker_cmd(host, "build", args = dockerfolder, docker_opts = docker_opts, wait = wait, ...)
}

#shorthand method for creating a local docker Image based on either an existing dockerfile or a Dockerfile object
#
create_localDockerImage = function(x, image_name = strsplit(tempfile(pattern = "containerit_test", tmpdir = ""),"/")[[1]][2], no_cache=FALSE){
  if(is.character(x))
    docker_build(harbor::localhost, dockerfolder = x,new_image = image_name, wait = TRUE)
  if(inherits(x, "Dockerfile")){
    ...
    docker_build(harbor::localhost, dockerfolder = tempdir,new_image = image_name, wait = TRUE, no_cache = no_cache)
...
obtain_localSessionInfo = function(expr = c(), vanilla=FALSE, local_tempfile = tempfile(pattern = "rdata-sessioninfo")){
  ...
  message("Creating R session with the following arguments:\n\t Rscript ",paste(expr,collapse = " ")) 
...
obtain_dockerSessionInfo = function(docker_image, expr = c(), vanilla=FALSE, docker_tempdir = "/tmp/containerit_temp", local_tempdir=tempfile(pattern = "dir")
  ...
  container = harbor::docker_run(harbor::localhost,image = docker_image, cmd=cmd , docker_opts = volume_opt)
  ...
  if(!file.exists(local_docker_tempfile))
    stop("Sessioninfo was not written to file (it does not exist): ", local_docker_tempfile)
  
  message("Wrote sessioninfo from Docker to this tempfile:", local_docker_tempfile)
