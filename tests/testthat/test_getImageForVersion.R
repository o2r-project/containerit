...
test_that("the closes version is returned if the one does not exist", {
  image <- getImageForVersion("3.2.99")
  expect_equal(as.character(image), "FROM rocker/r-ver:3.3.0")
})

test_that("there is an informative warning if nearest is enabled", {
  expect_warning(getImageForVersion("3.2.99", nearest = TRUE), "3.3.0")
...
test_that("there is an informative warning if nearest is disabled", {
  ...
  expect_warning(getImageForVersion("3.2.99", nearest = FALSE), "3.3.0")
})
