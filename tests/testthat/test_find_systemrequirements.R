...
test_that("system requirements for CRAN packages can be determinded OFFLINE", {
  deps <- .find_system_dependencies("sp", platform = .debian_platform, soft = TRUE, offline = TRUE)
  deps_expected <-
    c("libproj-dev", "libgdal-dev", "gdal-bin", "libgeos-dev")
  expect_true(all(deps %in% deps_expected))
  expect_true(all(deps_expected %in% deps))

  deps <- .find_system_dependencies("sp", platform = .debian_platform, soft = FALSE, offline = TRUE)
