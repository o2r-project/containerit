...
test_that("the same base packages are attached locally and in Docker ",{
  ...
  expect_true(all(local_sessionInfo$basePkgs %in% docker_sessionInfo$basePkgs))
  expect_true(all(docker_sessionInfo$basePkgs %in% local_sessionInfo$basePkgs))
})
...
test_that("the packages are loaded via Namespace locally and in Docker ",{
  ...
  local_versions = sapply(local_sessionInfo$loadedOnly, function(x) x$Version)
