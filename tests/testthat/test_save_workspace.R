...
context("Save workspace and R objects (save_image - argument): https://github.com/o2r-project/containerit/issues/48")
...
test_that("A workspace image can be containerized", {
  ...
  expect_true(file.exists(".RData"), "The expected workspace image '.RData' file was not writen dor working directory.")
  attach(".RData")
  ...
  expect_true(expected_instruction %in% generated_instructions, 
              paste("Expected a copy instruction after setting the workdir as follows:",expected_instruction))
...
test_that("A workspace image can be containerized given an object list and save-arguments", {
  ...
  expected_instruction <- toString(Copy(targetfile, targetfile))
  generated_instructions <- sapply(inst[copy_sel], toString)
  expect_true(expected_instruction %in% generated_instructions, 
              paste("Expected a copy instruction after setting the workdir as follows:", expected_instruction))
  #if you test-build the dockerfile, you should find the object original_sessionInfo loaded into the workspace
